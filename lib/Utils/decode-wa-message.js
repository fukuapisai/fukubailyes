...
const extractAddressingContext = (stanza) => {
    ...
    else if (WABinary_1.isPnUser(sender)) { 
...
const storeMappingFromEnvelope = async (stanza, sender, decryptionJid, repository, logger) => {
    ...
    if (senderAlt && WABinary_1.isLidUser(senderAlt) && WABinary_1.isPnUser(sender) && decryptionJid === sender) {
        try {
        ...
        async decrypt() {
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    decryptables += 1
                    let msgBuffer
                    const user = WABinary_1.isPnUser(sender) ? sender : author
