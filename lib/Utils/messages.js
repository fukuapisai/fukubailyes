...
const generateWAMessageContent = async (message, options = {}) => {
    ...
    if ('sections' in message && !!message.sections) {
    	...
    	// Process buttons to support iOS webview functionality
    	const processedButtons = message.interactiveButtons.map((button, index) => {
    		// Check if it's a URL button (support both 'type' and 'name' properties)
    		const isUrlButton = button.type === 'cta_url' || button.name === 'cta_url';
    		// Check if it's an open_webview button (iOS-specific handling)
    		const isOpenWebviewButton = button.name === 'open_webview';
    		
    		if (isUrlButton || isOpenWebviewButton) {
    			// Create buttonParamsJson if it doesn't exist
    			let buttonParams = {};
    			
    			if (button.buttonParamsJson) {
    				try {
    					buttonParams = typeof button.buttonParamsJson === 'string' 
    			...
    			// Extract URL from button structure - handle open_webview format
    			let buttonUrl = '';
    			let displayText = '';
    			
    			if (isOpenWebviewButton) {
				// Handle open_webview button format - check multiple possible URL locations
				if (buttonParams.url) {
					// Direct URL in buttonParams (most common format)
					buttonUrl = buttonParams.url;
				} else if (buttonParams.link && buttonParams.link.url) {
					// Nested URL in link object
					buttonUrl = buttonParams.link.url;
				}
				displayText = buttonParams.display_text || buttonParams.title || 'Open';
    			} else {
    				// Handle regular cta_url button format
    				buttonUrl = button.url || buttonParams.url || buttonParams.landing_page_url || '';
    				displayText = button.buttonText?.displayText || buttonParams.display_text || 'Click Here';
    			}
    			
    			// Add iOS webview support parameters
			const iosCompatibleParams = {
				display_text: displayText,
				url: buttonUrl,
				landing_page_url: buttonParams.landing_page_url || buttonUrl,
				webview_presentation: buttonParams.webview_presentation || null,
				webview_interaction: buttonParams.webview_interaction !== undefined ? buttonParams.webview_interaction : true,
				payment_link_preview: buttonParams.payment_link_preview !== undefined ? buttonParams.payment_link_preview : false,
				// Add merchant_url for iOS compatibility
				merchant_url: buttonParams.merchant_url || buttonUrl
			};
    			
    			// For open_webview buttons, add in_app_webview support
    			if (isOpenWebviewButton && buttonParams.link && buttonParams.link.in_app_webview !== undefined) {
    				iosCompatibleParams.in_app_webview = buttonParams.link.in_app_webview;
    			}
    			
    			return {
    				name: 'cta_url',
    				buttonParamsJson: JSON.stringify(iosCompatibleParams)
    			};
    		}
    		
    		// For non-URL buttons, return as-is
    		return button;
    	});
    	
    	// Generate messageParamsJson for iOS webview support
    	let messageParamsJson = message.messageParamsJson;
    	const hasUrlButtons = message.interactiveButtons.some(btn => 
    		btn.type === 'cta_url' || btn.name === 'cta_url' || btn.name === 'open_webview'
    	);
    	
    	if (!messageParamsJson && hasUrlButtons) {
    		const urlButtons = message.interactiveButtons.filter(btn => 
    			btn.type === 'cta_url' || btn.name === 'cta_url' || btn.name === 'open_webview'
    		);
    		const tapTargetList = urlButtons.map((button, index) => {
    			...
    			if (button.name === 'open_webview') {
    				// Handle open_webview button format
    				if (button.buttonParamsJson) {
    					try {
    						const buttonParams = typeof button.buttonParamsJson === 'string' 
    							? JSON.parse(button.buttonParamsJson) 
    							: button.buttonParamsJson;
    						if (buttonParams.link && buttonParams.link.url) {
    							canonicalUrl = buttonParams.link.url;
    						} else if (buttonParams.url) {
    							canonicalUrl = buttonParams.url;
    						}
    					} catch (e) {
    						canonicalUrl = '';
    					}
				...
				canonicalUrl = button.url;
			} else if (button.buttonParamsJson) {
				try {
					const buttonParams = typeof button.buttonParamsJson === 'string' 
						? JSON.parse(button.buttonParamsJson) 
						: button.buttonParamsJson;
					// Prioritize landing_page_url for iOS compatibility (matches working iOS messages)
					canonicalUrl = buttonParams.landing_page_url || buttonParams.url || '';
				} catch (e) {
					canonicalUrl = '';
				}
			}
      ...
      // Add iOS-compatible contextInfo with dataSharingContext
...
const { createIOSWebviewMessage, createWebviewButton } = require('./ios-webview-helper');
